...
maybePrompt :: IO t -> IO (Maybe t)
maybePrompt p = Just `fmap` p
--  case nonInteractive flags of
--    Flag True -> return Nothing
--    _         -> 
-- | Create a prompt with optional default value that returns a
--   Number.
promptNum :: (Fractional a, Read a, Show a) => String -> Maybe a -> IO a
promptNum = promptDefault' (readMaybe) show

-- | Create a prompt with optional default value that returns a
--   Number.
promptInt :: String -> Maybe Int -> IO Int
promptInt = promptDefault' (\a -> readMaybe a :: Maybe Int) show
